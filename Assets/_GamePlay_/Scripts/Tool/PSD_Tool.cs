...
public class PSD_Tool : MonoBehaviour
{
    ...
    public void FindAllChildGameObjects()
    {
        ...
        SpriteRenderer[] childRenderers = holder.GetComponentsInChildren<SpriteRenderer>(true);
        for (int i = 0; i < childRenderers.Length; i++)
        {
            SpriteRenderer renderer = childRenderers[i];
            if (renderer.gameObject == holder)
            {
                continue;
            }

            childGameObjects.Add(renderer.gameObject);
        }

        Debug.Log($"Found {childGameObjects.Count} child GameObject(s) with a SpriteRenderer.", this);
    }
