...
public class TransparentBackgroundScreenshotRecorder : MonoBehaviour
{
    ...
    [Tooltip("Scales game time while baking. Lower values make baked animation slower.")]
    [Min(0.01f)] public float bakeTimeScale = 0.5f;
    #endregion
    #region private fields
    private string folderName = "/Assets/Assets/";
    ...
    void Awake()
    {
        ...
        Time.timeScale = originalTimescaleTime * bakeTimeScale;
    ...
    void CalculateOutputTexture()
    {
        ...
        for (int y = 0; y < textureTransparentBackground.height; ++y)
        {
            ...
            {
                Color black = textureBlack.GetPixel(x, y);
                Color white = textureWhite.GetPixel(x, y);

                float alphaR = 1.0f - (white.r - black.r);
                float alphaG = 1.0f - (white.g - black.g);
                float alphaB = 1.0f - (white.b - black.b);

                // Dùng max thay vì trung bình: tránh màu bão hòa đơn kênh (đỏ, xanh lá,
                // xanh dương...) bị kéo alpha xuống thấp bởi các kênh còn lại gần 0,
                // dẫn đến màu bị nhạt/mờ khi composite.
                float alpha = Mathf.Max(alphaR, alphaG, alphaB);
                ...
                {
                    // Reconstruct từng kênh độc lập để tránh over-brighten
                    color.r = black.r / alpha;
                    color.g = black.g / alpha;
                    color.b = black.b / alpha;
                }
                color.a = alpha;
