...
public class ItemDragRaycastTarget : MonoBehaviour
{
    ...
    private void HandleBeginDrag()
    {
        ...
        currentTarget = null;
        processedCleaningTargets.Clear();
        originalDefaultTarget = ownerItem != null && ownerItem.itemMoveToTarget != null
            ? ownerItem.itemMoveToTarget.defaultTarget
            : null;
    }

    private void EndCleaningTargets()
    {
        for (int i = 0; i < processedCleaningTargets.Count; i++)
        {
            var target = processedCleaningTargets[i];
            if (target != null) target.EndProcess();
        }
        processedCleaningTargets.Clear();
    }
    ...
    private List<CleaningTarget> processedCleaningTargets = new List<CleaningTarget>();
    ...
    private void FindTargetUnderPointer()
    {
        ...
        // --- Handle Cleaning Targets ---
        for (int i = 0; i < hits.Length; i++)
        {
            var hit = hits[i];
            CleaningTarget cTarget = hit.collider.GetComponentInParent<CleaningTarget>();
            if (cTarget != null && cTarget.CanProcess(ownerItem))
            {
                if (!processedCleaningTargets.Contains(cTarget))
                {
                    processedCleaningTargets.Add(cTarget);
                }
                cTarget.Process(raycastPoint != null ? raycastPoint.position : transform.position);
            }
        }
        // -------------------------------
