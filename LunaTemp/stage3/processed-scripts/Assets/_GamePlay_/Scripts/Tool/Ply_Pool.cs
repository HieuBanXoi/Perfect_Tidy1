...
public class Ply_Pool : Ply_Singleton<Ply_Pool>
{
    ...
    public Ply_GameUnit SpawnDirect(PoolType poolType, [Bridge.Ref] Vector3 pos, [Bridge.Ref] Quaternion rot)
    {
        Ply_GameUnit prefab = GetPrefab(poolType);
        if (prefab == null)
        {
    ...
    public T SpawnDirect<T>(PoolType poolType, [Bridge.Ref] Vector3 pos, [Bridge.Ref] Quaternion rot) where T : Ply_GameUnit
    {
